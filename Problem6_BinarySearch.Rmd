...
- you have `ToBeSorted <- sort(as.integer( runif(100, min = 1, max = 99) ))`
- Implement `binary search ` find where `T` is in `ToBeSorted`
- In this excersise, set `T <- as.integer( runif(1, min = 1, max = 99)`.
- In the case `ToBeSorted` dose not have the same number as `T`, print('can't find')

# Sample Answer


```{r}
binary <- function(ToBeSorted,Target){
  
  N <- length(ToBeSorted) #Number of factor
  Start <- 1
  End   <- N
  
  while(Start<End){
    Mid <- as.integer((Start + End)/2)
    if(ToBeSorted[Mid] == Target){
      return(Mid)
    }else if(Target < ToBeSorted[Mid]){
      End <- Mid 
    }else{
      Start <- Mid + 1
    }
...
ToBeSorted <- sort(as.integer( runif(100, min = 1, max = 99) ))
Target <-  as.integer( runif(1, min = 1, max = 99) )

print(Target) 
ToBeSorted[binary(ToBeSorted,Target)]
print(ToBeSorted)
```
